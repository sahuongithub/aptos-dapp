...
const DepositButton = () => {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const transaction = {
        data: {
          function: "0xf02e42e167e86430855e112267405f0bb4bb6a8fed16cd7e4e4a339ec7341f73::VaultFactory::deposit",
          functionArguments: [normalizedAddress, amountInOctas.toString()],
        },
        options: {
          maxGasAmount: 20000,      // Sufficient gas for deposit transactions
      ...
      if (response) {
        ...
        setSuccess(`Successfully deposited ${amount} APT! Transaction: ${response.hash}`);
        console.log("Deposit successful:", response);
        
        // Clear inputs after successful deposit
        setLeaderAddress("");
        setAmount("");
      }
    } catch (err) {
      ...
      let errorMessage = "Failed to deposit to vault";
      
      if (err.message) {
        if (err.message.includes("insufficient")) {
          errorMessage = "Insufficient balance for transaction fees or deposit amount.";
    ...
    <div style={{ marginBottom: "15px" }}>
      ...
      {!connected && (
        <p style={{
          ...
          Connect wallet to deposit to vault
        </p>
      )}
